...
def get_data_loaders(config, train_batch_size, exp_id=0):
    ...
    val_dataset = PolypDataset(config, exp_id, 'val')
    val_loader = torch.utils.data.DataLoader(val_dataset)

    if config['test_ratio']:
        test_dataset = PolypDataset(config, exp_id, 'test')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--database', default='POLYP', type=str,
                        help='database name (default: POLYP)')
